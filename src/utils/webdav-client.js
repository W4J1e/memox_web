...
export class WebDavClient {
  ...
  // Download a binary attachment in a single GET. The EdgeOne Cloud Function now
  // streams the upstream body back (no in-memory buffering), so one request is
  // enough and the old 504 timeout on large images no longer applies. Going from
  // the previous HEAD + Range-chunked fetch to a single GET also lets callers
  // parallelize downloads, which is what cuts attachment sync time.
